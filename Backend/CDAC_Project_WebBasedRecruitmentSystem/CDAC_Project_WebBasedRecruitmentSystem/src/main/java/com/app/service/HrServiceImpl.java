...
//	}
	...
	public HrResponse getHrDetails() {
	    ...
	    HREntity hrEntity = hrRepo.findByUser(userEntity).orElseThrow();
	    hrEntity.setUser(userEntity);

	    HrResponse hrResponse = mapper.map(hrEntity, HrResponse.class);

	    // Set the image URL directly from the entity
	    hrResponse.setImageURL(hrEntity.getImageURL());
